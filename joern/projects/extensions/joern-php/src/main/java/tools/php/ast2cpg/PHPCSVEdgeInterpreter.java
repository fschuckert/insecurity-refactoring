...
public class PHPCSVEdgeInterpreter implements CSVRowInterpreter {
	...
	public long handle(KeyedCSVRow row, ASTUnderConstruction ast) throws InvalidCSVFile {
		...
		// then do not add the childnum property to ASTNodes in node interpreter any
		// longer,
		// then introduce some NumberFormatException handling here.
		// int childnum =
		// Integer.parseInt(row.getFieldForKey(PHPCSVEdgeTypes.CHILDNUM));
		int childnum = Integer.parseInt(endNode.getProperty(PHPCSVNodeTypes.CHILDNUM.getName()));

		// Special treatment for closures: they are expressions, so we create a
		// ClosureExpression to hold them
		// We cannot do this in the PHPCSVNodeInterpreter, since CSV2AST expects an
		// instance of PHPFunctionDef
		// for the first row of the CSVAST that it converts. (Closure is an instance of
		// PHPFunctionDef and thus
		...
		if (1 == errno)
			throw new InvalidCSVFile("While trying to handle row " + row.toString() + ": Invalid childnum " + childnum
					+ " for start node type " + type + ".");
		else if (2 == errno)
			throw new InvalidCSVFile(
					"While trying to handle row " + row.toString() + ": Cannot add child to leaf node " + type + ".");
	...
	private int handleFunction(FunctionDef startNode, ASTNode endNode, int childnum) {
		...
		// 0 1 2 3 4
		// AST 70: name, docComment, params, stmts, returnType
		switch (childnum) {
		case 0: // params child
//				startNode.setParameterList((ParameterList)endNode);
			// fixed: name
			StringExpression stringExpression = (StringExpression) endNode;
			startNode.setName(stringExpression.getEscapedCodeStr());
//                                startNode.set
			break;
		case 1: // NULL child
				// fixed: docComment
...
//					startNode.setReturnType((Identifier)endNode);
	...
	private int handleClosure(Closure startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // name
                        StringExpression name = (StringExpression)endNode;
                        startNode.setName(name.getEscapedCodeStr());
			break;
		case 1: 
		...
		case 3: // uses child: either ClosureUses or NULL node
			...
			startNode.setContent((CompoundStatement) endNode);
                ...
                case 5:
                        // returnType child: either Identifier or NULL node
			if (endNode instanceof NullNode)
				startNode.addChild(endNode);
			else
				startNode.setReturnType((Identifier) endNode);
...
//					startNode.setReturnType((Identifier)endNode);
	...
	private int handleClass(ClassDef startNode, ASTNode endNode, int childnum) {
		...
		// 0 1 2 3 4
		// AST 70: name, docComment, extends, implements, stmts
...
//				startNode.setReturnType((Identifier)endNode);
		...
		default:
			...
			/*
			 * Old for classes case 0: // extends child: either Identifier or NULL node if(
			 * endNode instanceof NullNode) startNode.addChild(endNode); else
			 * startNode.setExtends((Identifier)endNode); break; case 1: // implements
			 * child: either IdentifierList or NULL node if( endNode instanceof NullNode)
			 * startNode.addChild(endNode); else
			 * startNode.setImplements((IdentifierList)endNode); break; case 2: // toplevel
			 * child startNode.setTopLevelFunc((TopLevelFunctionDef)endNode); break;
			 * 
			 * default: errno = 1;
			 */
		}
	...
	private int handleVariable(Variable startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // name child
			startNode.setNameExpression(getExpression(endNode));
			break;
	...
	private int handleUnpack(UnpackExpression startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // expr child
			startNode.setExpression(getExpression(endNode));
			break;
	...
	private int handleCast(CastExpression startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // expr child
			startNode.setCastExpression(getExpression(endNode));
			break;
	...
	private int handleIsset(IssetExpression startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // var child
			startNode.setVariableExpression(getExpression(endNode));
			break;
	...
	private int handleShellExec(ShellExecExpression startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // expr child
			startNode.setShellCommand(getExpression(endNode));
			break;
	...
	private int handleIncludeOrEval(IncludeOrEvalExpression startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // expr child
			startNode.setIncludeOrEvalExpression(getExpression(endNode));
			break;
	...
	private int handleYieldFrom(YieldFromExpression startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // expr child
			startNode.setFromExpression(getExpression(endNode));
			break;
	...
	private int handleReturn(ReturnStatement startNode, ASTNode endNode, int childnum) {
		...
		case 0: // expr child: Expression or null node
			if (endNode instanceof NullNode)
				...
				startNode.setReturnExpression(getExpression(endNode));
			break;
	...
	private int handleEcho(EchoStatement startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // expr child
			startNode.setEchoExpression(getExpression(endNode));
			break;
	...
	private int handleThrow(ThrowStatement startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // expr child
			startNode.setThrowExpression(getExpression(endNode));
			break;
	...
	private int handleArrayIndexing(ArrayIndexing startNode, ASTNode endNode, int childnum) {
		...
		case 0: // expr child: Expression node
			startNode.setArrayExpression(getExpression(endNode));
			break;
		case 1: // dim child: Expression or NULL node
			if (endNode instanceof NullNode)
				startNode.addChild(endNode);
			else
				startNode.setIndexExpression(getExpression(endNode));
			break;
	...
	private int handleProperty(PropertyExpression startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // expr child: Expression node
			startNode.setObjectExpression(getExpression(endNode));
			break;
		case 1: // prop child: Expression node
			startNode.setPropertyExpression(getExpression(endNode));
			break;
	...
	private int handleStaticProperty(StaticPropertyExpression startNode, ASTNode endNode, int childnum) {
		...
		case 0: // class child: Expression node
			startNode.setClassExpression(getExpression(endNode));
			break;
		case 1: // prop child: Expression node
			startNode.setPropertyExpression(getExpression(endNode));
			break;
	...
	private int handleCall(CallExpressionBase startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // expr child: Expression node
			startNode.setTargetFunc(getExpression(endNode));
			break;
	...
	private int handleAssign(AssignmentExpression startNode, ASTNode endNode, int childnum) {
		...
		case 0: // var child: Expression node
			startNode.setLeft(getExpression(endNode));
			break;
		case 1: // expr child: Expression node
			startNode.setRight(getExpression(endNode));
			break;
	...
	private int handleArrayElement(ArrayElement startNode, ASTNode endNode, int childnum) {
		...
		case 0: // value child: Expression node
                        startNode.setValue(getExpression(endNode));
			break;
		case 1: // key child: Expression or null node
			if (endNode instanceof NullNode)
				startNode.addChild(endNode);
			else
				startNode.setKey(getExpression(endNode));
			break;
	...
	private int handleNew(NewExpression startNode, ASTNode endNode, int childnum) {
		...
		case 0: // class child: ClassDef or Expression node
			if (endNode instanceof ClassDef) // the child is a class used as an expression
				...
				startNode.setTargetClass(getExpression(endNode));
			break;
	...
	private int handleInstanceof(InstanceofExpression startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // expr child: Expression node
			startNode.setInstanceExpression(getExpression(endNode));
			break;
	...
	private int handleStaticVariable(StaticVariableDeclaration startNode, ASTNode endNode, int childnum) {
		...
		case 1: // default child: Expression or null node
			if (endNode instanceof NullNode)
				...
				startNode.setDefault(getExpression(endNode));
			break;
	...
	private int handleWhile(WhileStatement startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // cond child
			startNode.setCondition(getExpression(endNode));
			break;
		case 1: // stmts child: Statement or Expression or null node
			if (endNode instanceof NullNode)
				...
				startNode.setStatement(createExpressionStatement(getExpression(endNode)));
			else
	...
	private int handleMethodCall(MethodCallExpression startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // expr child: Expression node
			startNode.setTargetObject(getExpression(endNode));
			break;
	...
	private int handleConditional(ConditionalExpression startNode, ASTNode endNode, int childnum) {
		...
		case 1: // trueExpr child: Expression or null node
			if (endNode instanceof NullNode)
				...
				startNode.setTrueExpression(getExpression(endNode));
			break;
		case 2: // falseExpr child: Expression node
			startNode.setFalseExpression(getExpression(endNode));
			break;
	...
	private int handleCatch(CatchStatement startNode, ASTNode endNode, int childnum) {
		...
		case 0: // exception child: Identifier node
			...
			IdentifierList tmp = (IdentifierList) endNode;
			if (tmp.size() != 1) {
				System.out.println("debug");
			} else {
				startNode.setExceptionIdentifier(tmp.getIdentifier(0));
			}

			// startNode.setExceptionIdentifier();
	...
	private int handleFor(ForStatement startNode, ASTNode endNode, int childnum) {
		...
		case 0: // init child: ExpressionList or null node
			if (endNode instanceof NullNode)
				...
				startNode.setForInitExpression(getExpression(endNode));
			break;
		...
		case 2: // loop child: ExpressionList or null node
			if (endNode instanceof NullNode)
				...
				startNode.setForLoopExpression(getExpression(endNode));
			break;
	...
	private int handleForEach(ForEachStatement startNode, ASTNode endNode, int childnum) {
		...
		switch (childnum) {
		case 0: // expr child: Expression node
			startNode.setIteratedObject(getExpression(endNode));
			break;
		case 1: // value child: Expression node
			startNode.setValueExpression(getExpression(endNode));
			break;
		case 2: // key child: Expression or null node
			if (endNode instanceof NullNode)
				startNode.addChild(endNode);
			else
				startNode.setKeyExpression(getExpression(endNode));
			break;
		case 3: // stmts child: Statement or Expression or null node
			if (endNode instanceof NullNode)
				startNode.addChild(endNode);
        ...
        private Expression getExpression(ASTNode endNode){
            if(endNode instanceof Expression){
                return (Expression) endNode;
            }
            else  {
//                    ASTNode aSTNode = ()
                //TODO check if just adding the child is ok or not!
                if(endNode.getChildCount() == 1){
                    
                    return (Expression)endNode.getChild(0);
                }
                else{
                    Expression crash = (Expression) endNode; // should crash!
                }
            }
            return null;
        }
	...
	private int handleArgumentList(ArgumentList startNode, ASTNode endNode, int childnum) {
                // TODO check
                startNode.addArgument(getExpression(endNode));
	...
	private int handleEncapsList(EncapsListExpression startNode, ASTNode endNode, int childnum) {
		startNode.addElement(getExpression(endNode));

		return 0;
	}

	private int handleExpressionList(ExpressionList startNode, ASTNode endNode, int childnum) {
		startNode.addExpression(getExpression(endNode));
	...
	private ExpressionStatement createExpressionStatement(Expression expression) {
		...
		// the ExpressionStatement gets the same NODE_ID as the Expression, this way CFG
		// creation
		// can handle the ExpressionStatement, but the end node that we reference when
		// writing
	...
	private ClosureExpression createClosureExpression(Closure closure) {
		...
		// the ClosureExpression gets the same NODE_ID as the Closure, this way CFG
		// creation
		// can handle the ClosureExpression, but the end node that we reference when
		// writing
	...
	/**
	 * Creates a ClassExpression wrapper around a PHPClassDef node (for anonymous
	 * classes.)
	 */
	private ClassExpression createClassExpression(ClassDef classDef) {
		...
		// the ClassExpression gets the same NODE_ID as the PHPClassDef, this way CFG
		// creation
		// can handle the ClassExpression, but the end node that we reference when
		// writing
