...
public class CSVFunctionExtractor
{
	...
	CSV2AST csv2ast = new PHPCSV2AST();
        Queue<Long> classes = new LinkedList<>();
		...
		while( nodeReader.hasNextRow())
		{
			...
			if( currType.equals(PHPCSVNodeTypes.TYPE_TOPLEVEL) &&
				...
				continue;
                        ...
                        if( currType.equals(PHPCSVNodeTypes.TYPE_CLASS)){
//                            classes.add(currNodeRow.)
                            String idStr = currNodeRow.getFieldForKey(new CSVKey("ID"));
			...
			if( csvStack.empty()){
				throw new InvalidCSVFile( "nodeReader, line " + nodeReader.getCurrentLineNumber() + ": " + "No toplevel node of a file to initialize top-level code was found.");
                        }
			...
			if( finishedFunctions < 0){
				throw new InvalidCSVFile( "nodeReader, line " + nodeReader.getCurrentLineNumber() + ": " + "funcid " + currFuncId + " has never been initialized by a function declaration.");
                        }
